...
class ModelArgs:
    ...
    def from_name(cls, name: str):
        ...
        # fuzzy search
        print("??", transformer_configs, name)
        config = [config for config in transformer_configs if config.lower()
                  in str(name).lower()]
...
transformer_configs = {
    ...
    "34B": dict(n_layer=48, n_head=64, dim=8192, vocab_size=32000, n_local_heads=8, intermediate_size=22016, rope_base=1000000),
...
class Transformer(nn.Module):
    ...
    def setup_caches(self, max_batch_size, max_seq_length):
        ...
        self.freqs_cis = precompute_freqs_cis(
            self.config.block_size, self.config.dim // self.config.n_head, self.config.rope_base, dtype, self.config.rope_scaling)
...
class Attention(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        total_head_dim = (config.n_head + 2 *
    ...
    def forward(self, x: Tensor, freqs_cis: Tensor, mask: BlockMask, input_pos: Optional[Tensor] = None) -> Tensor:
        ...
        y = flex_attention(q, k, v, block_mask=mask, enable_gqa=(
