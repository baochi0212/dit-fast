...
class OmniGenPipeline:
    def __init__(
        ...
        if device is None:
            if torch.cuda.is_available():
                ...
                logger.info(
                    "Don't detect any available GPUs, using CPU instead, this may take long time to generate image!!!")
    ...
    def from_pretrained(cls, model_name, vae_path: str = None):
        ...
        model = OmniGen.from_pretrained(model_name).to(dtype=torch.bfloat16)
        processor = OmniGenProcessor.from_pretrained(model_name)

        if os.path.exists(os.path.join(model_name, "vae")):
            vae = AutoencoderKL.from_pretrained(
                os.path.join(model_name, "vae")).to(torch.bfloat16)
        elif vae_path is not None:
            vae = AutoencoderKL.from_pretrained(
                vae_path).to(device).to(torch.bfloat16)
        else:
            logger.info(
                f"No VAE found in {model_name}, downloading stabilityai/sdxl-vae from HF")
            vae = AutoencoderKL.from_pretrained(
                "stabilityai/sdxl-vae").to(device, dtype=torch.bfloat16)
    ...
    def vae_encode(self, x, dtype):
        if self.vae.config.shift_factor is not None:
            ...
            x = self.vae.encode(x).latent_dist.sample().mul_(
                self.vae.config.scaling_factor)
        x = x.to(dtype)
        return x
    ...
    def __call__(
        ...
        if use_input_image_size_as_output:
            assert isinstance(prompt, str) and len(
                input_images) == 1, "if you want to make sure the output image have the same size as the input image, please only input one image instead of multiple input images"
        else:
        ...
        input_data = self.processor(prompt, input_images, height=height, width=width, use_img_cfg=use_img_guidance,
                                    separate_cfg_input=separate_cfg_infer, use_input_image_size_as_output=use_input_image_size_as_output)
        ...
        if seed is not None:
            ...
            generator = None
        latents = torch.randn(num_prompt, 4, latent_size_h,
                              latent_size_w, device=self.device, generator=generator)
        ...
        model_kwargs = dict(input_ids=self.move_to_device(input_data['input_ids']),
                            ...
                            attention_mask=self.move_to_device(
                                input_data["attention_mask"]),
                            position_ids=self.move_to_device(
        ...
        samples = scheduler(latents, func, model_kwargs,
